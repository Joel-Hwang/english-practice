...
from service.userService import registerUser, login, changePassword
from model.user import UserCreate, User, ChangePassword
...
class TestUserService(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_registerUser_success(self, mock_hashpw, mock_insertUser, mock_findUserById):
        ...
        await registerUser(user_create)
    ...
    async def test_registerUser_missing_fields(self):
        with self.assertRaises(HTTPException) as cm:
            await registerUser(UserCreate(id='', password='password123'))
        self.assertEqual(cm.exception.status_code, 422)
        self.assertEqual(cm.exception.detail, 'ID and password are required.')

        with self.assertRaises(HTTPException) as cm:
            await registerUser(UserCreate(id='test@example.com', password=''))
        self.assertEqual(cm.exception.status_code, 422)
        self.assertEqual(cm.exception.detail, 'ID and password are required.')
    ...
    async def test_login_success(self, mock_checkpw, mock_findUserById):
        ...
        user_login = await login(login_data)
    ...
    async def test_login_invalid_id(self, mock_findUserById):
        ...
        with self.assertRaises(HTTPException) as cm:
            await login(UserCreate(id='nonexistent@example.com', password='password123'))
    ...
    async def test_login_missing_fields(self):
        with self.assertRaises(HTTPException) as cm:
            await login(UserCreate(id='', password='password123'))
        self.assertEqual(cm.exception.status_code, 422)
        self.assertEqual(cm.exception.detail, 'ID and password are required.')
    ...
    @patch('repository.userRepository.findUserById', new_callable=AsyncMock)
    @patch('repository.userRepository.updatePassword', new_callable=AsyncMock)
    @patch('bcrypt.checkpw')
    @patch('bcrypt.hashpw')
    async def test_changePassword_success(self, mock_hashpw, mock_checkpw, mock_updatePassword, mock_findUserById):
        mock_findUserById.return_value = {
            "id": "test@example.com", 
        ...
        change_password_data = ChangePassword(id='test@example.com', oldPassword='old_password', newPassword='new_password')
        await changePassword(change_password_data)
        
        mock_updatePassword.assert_called_once()
